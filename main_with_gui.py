...
class CameraInitializer(QRunnable):
    def __init__(self):
        super().__init__()
        self.signals = CameraInitializerSignals()
        self.cap = None
        self.meanMTX = None
        self.meanDIST = None
    ...
    def run(self):
        try:
            ...
            if not self.cap.isOpened():
                raise ValueError(f"Could not open camera {CAMERA_NUMBER}")
            ...
            self.signals.initialized.emit(True)
...
# Create and start camera initializer
initializer = CameraInitializer()
thread_pool.start(initializer)

# Wait for initialization to complete
initializer.signals.initialized.connect(lambda success: 
    print(f"Camera initialization {'completed' if success else 'failed'}"))

# Store initialized camera and calibration data
cap = initializer.cap
meanMTX = initializer.meanMTX
meanDIST = initializer.meanDIST
...
class MainWindow(QMainWindow):
    ...
    def start_calibration(self):
        ...
        # Assuming you already have the camera capture object available as `cap`
        rows = 6
        columns = 9
        square_size = 30  # Size of the calibration squares
        runs = 1  # Number of runs for calibration
        meanMTX, meanDIST, uncertaintyMTX, uncertaintyDIST = CalibrationWithUncertainty.calibrateCamera(
